...
`components/ui/**`, `package.json`, `node_modules/`, `jsconfig.json`, and
`vite.config.js` — their contents are reproduced below, so opening them only
spends turns.
...
- **Backend** (`backend/requirements.txt`): `fastapi`, `uvicorn`, `motor`,
  ...
  `python-multipart`.
- **Absent**, install before use: `leaflet`, `openpyxl`, any
...
| `frontend/src/main.jsx` | already mounts `StrictMode` + `QueryClientProvider` + `BrowserRouter` — never edit, and never re-add any of the three in `App.jsx` (a second Router breaks routing; `<Routes>` in `App.jsx` just works) |
| `frontend/src/App.jsx` | the `<Routes>` table and nothing else — one `<Route>` per page, added in the same edit that creates the page. A page with no `<Route>` is unreachable, and any URL without a matching `<Route>` renders a **blank page** — `<Routes>` matches nothing and mounts nothing |
| `frontend/src/pages/*.jsx` | one screen per file, default-exported, imported into `App.jsx` as `@/pages/<Name>`; `Home.jsx` ships as the worked example |
| `frontend/src/lib/api.js` | `apiGet/apiPost/apiPut/apiPatch/apiDelete` over base `/api`, throwing `ApiError` |
...
FastAPI and Pydantic own runtime request/response validation. JavaScript consumes
the returned JSON directly, so assert important response shapes in backend or
browser tests rather than adding fake JSDoc types or duplicated type files.
...
```js
// frontend — call the relative path; assert important fields in a test
const thing = await apiGet(`/things/${id}`);   // → /api/things/<id>
```
...
### Auth apps: sessions are httpOnly cookies; `lib/session.js` owns the cache
...
- `Select`'s `onValueChange` gives a string (the wrapper coerces base-ui's
  `null`) — handle it as a string. And
...
|---|---|
| JavaScript has no compile-time response types | assert important response fields in backend or browser tests |
| do not add JSDoc types or hidden `checkJs` config | keep this an untyped JavaScript comparison arm |
...
The dev servers surface syntax and runtime errors as you write. These are facts for writing
...
- `recharts` + `react-is` are preinstalled; `leaflet` and
...
- **Backend import sanity** when the build is complete:
...
changing `.env`, `requirements.txt`, or `vite.config.js` — never at session start.
...
|---|---|
| `badge.jsx` | `Badge` (span) `variant`: default \| secondary \| destructive \| outline \| ghost \| link; `badgeVariants` |
| `button.jsx` | `Button` `variant`: default \| outline \| secondary \| ghost \| destructive \| link; `size`: default \| xs \| sm \| lg \| icon \| icon-xs \| icon-sm \| icon-lg; `buttonVariants` |
| `calendar.jsx` | `Calendar` (react-day-picker `DayPicker` props) + `buttonVariant` (ghost), `showOutsideDays` (true), `captionLayout` ("label"), `locale`; `CalendarDayButton` |
| `card.jsx` | `Card` `size`: default \| sm; `CardHeader/Title/Description/Action/Content/Footer` (div props) |
| `checkbox.jsx` | `Checkbox` = base-ui `Checkbox.Root` (`checked`, `defaultChecked`, `onCheckedChange`, `indeterminate`); indicator built in |
| `dialog.jsx` | `Dialog`, `DialogTrigger/Portal/Close/Overlay/Title/Description`; `DialogContent` + `showCloseButton` (true); `DialogHeader`, `DialogFooter` |
| `dropdown-menu.jsx` | `DropdownMenu` = base-ui `Menu.Root`; `…Trigger/Portal/Group/RadioGroup/Sub/SubTrigger/Separator/Label/Shortcut`; `…Content`, `…SubContent` (self-portal) + `align`, `alignOffset`, `side`, `sideOffset`; `…Item`/`…CheckboxItem`/`…RadioItem` + `inset`, `variant`: default \| destructive |
| `input.jsx`, `label.jsx`, `textarea.jsx` | `Input` / `Label` / `Textarea` — plain element props |
| `popover.jsx` | `Popover` = base-ui `Popover.Root`; `PopoverTrigger/Title/Description/Header`; `PopoverContent` (self-portals) + `align` ("center"), `alignOffset`, `side` ("bottom"), `sideOffset` (4) |
| `select.jsx` | `Select` (wrapped `Select.Root`), `SelectGroup/Value/Content/Label/Item/Separator/ScrollUpButton/ScrollDownButton`; `SelectTrigger` + `size`: sm \| default |
| `sheet.jsx` | `Sheet`, `SheetTrigger/Close/Portal/Overlay/Title/Description/Header/Footer`; `SheetContent` + `side`: top \| right (default) \| bottom \| left, `showCloseButton` (true) |
| `sonner.jsx` | `Toaster` — supports `position`, `richColors`, and other sonner options; theme from `next-themes`. Mount once, then `toast()` from `sonner` |
| `table.jsx` | `Table` (in an `overflow-x-auto` div), `TableHeader/Body/Footer/Row/Head/Cell/Caption` |
| `tabs.jsx` | `Tabs` + `orientation`: horizontal (default) \| vertical; `TabsList` + `variant`: default \| line; `TabsTrigger`, `TabsContent`; `tabsListVariants` |
